...
> [!NOTE]
> Cross-pane move and resize requests are sent over OSC user vars and handled inside Wezterm.
> `wezterm cli` is still used in the background to refresh pane-edge and zoom caches, and to create
> new panes for `at_edge = 'split'`.

First, ensure that the `wezterm` CLI is on your `$PATH`, as it is still used by the integration for
background layout refreshes and pane splitting.
...
w.on('user-var-changed', function(window, pane, name, value)
  ...
  if parts[1] == 'move' and parts[2] ~= nil then
    window:perform_action({ ActivatePaneDirection = parts[2] }, pane)
  elseif parts[1] == 'resize' and parts[2] ~= nil then
    window:perform_action({ AdjustPaneSize = { parts[2], tonumber(parts[3]) or default_resize_amount } }, pane)
  end
end)
...
If you use the plugin loader path, `apply_to_config()` installs the `SMART_SPLITS_CMD` handler for you.
If you use a custom Wezterm config instead, you need an equivalent `user-var-changed` handler so Neovim can
signal cross-pane moves and resizes without spawning `wezterm cli` on each keypress.
